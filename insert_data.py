    ...
    for index, row in df.iterrows():
        product = Product(
            ...
            price=convert_to_float(
                row["actual_price"].replace("₹", "").replace(",", "")
            ),
            discount_price=convert_to_float(
                row["discount_price"].replace("₹", "").replace(",", "")
            ),
...
def add_user():
    db.session.add(User(name="user_1", email="user_1@eemi.com"))
    db.session.commit()
